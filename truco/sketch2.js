...
class TrucoGame {
  ...
  updateResponsiveLayout() {
    ...
    // Optimización específica para escritorio
    if (isLandscape) {
      // Tamaños de carta para escritorio
      const cardWidth = min(baseSize * 0.15, 130);
      const cardHeight = cardWidth * 1.5;
      this.updateAllCardSizes(cardWidth, cardHeight);
      
      // Espaciado y posicionamiento optimizado
      this.cardSpacing = min(windowWidth * 0.09, baseSize * 0.11);
      this.centerX = windowWidth * 0.35;
      this.centerY = windowHeight * 0.35;
      
      // Consola - definimos primero para usar como referencia
      ...
      // Posición vertical de manos ajustada - alineada con consola
      const cardHeight_px = cardHeight;
      this.computerHandY = this.logY;  // Alineado exactamente con la consola
      this.playerHandY = windowHeight * 0.75;    // Subido de 0.85 a 0.75 para PC
      ...
      // Mantener configuración móvil existente sin cambios
      const cardWidth = min(baseSize * 0.2, 160);
      const cardHeight = cardWidth * 1.5;
      this.updateAllCardSizes(cardWidth, cardHeight);
      ...
      this.playerHandY = windowHeight * 0.85;    // Bajado de 0.73 a 0.85 para móvil
      this.computerHandY = windowHeight * 0.12;   // Subido de 0.17 a 0.12 para móvil
  ...
  waitForPlayerTrucoResponse() {
    ...
    const startY = windowHeight * (isLandscape ? 0.3 : 0.4);

    if (!this.envidoResponseState) {
      // Primera respuesta: Quiero, No Quiero, etc.
      let options = ["Quiero", "No Quiero"];
      
      // Si es la primera mano y no se han jugado cartas, mostrar todas las opciones
      if (this.round === 1 && this.players[0].playedCards.length === 0) {
        if (this.players[0].hasFlor() && !this.players[0].hasShownFlor) {
          options = ["Quiero", "No Quiero", "Envido", "Real Envido", "Falta Envido", "Flor"];
        } else {
      ...
      if (isLandscape) {
        ...
        options.forEach((label, i) => {
          ...
          this.buttons.push(new Button(label, buttonX, startY, buttonWidth, buttonHeight));
        });
      } else {
        // En móvil: dos columnas
        const columns = 2;
      ...
      // Estos botones siempre en una fila
      const totalWidth = options.length * buttonWidth + buttonSpacing;
      const startX = (windowWidth - totalWidth) / 2;
  ...
  handlePlayerEnvidoResponse(response) {
    ...
    if (!this.envidoResponseState) {
      ...
      if (response === "Son buenas") {
        ...
        // Mostrar los puntos del envido
        this.envidoPoints = [playerPoints, this.aiEnvidoPoints];
        ...
        this.logEvent(`Jugador mostró ${playerPoints}`);
  ...
  handleAIEnvidoResponse() {
    ...
    if (aiPoints >= 28 || (aiPoints >= 25 && Math.random() < 0.7)) {
      ...
      if (aiPoints > playerPoints) {
        this.logEvent(`Computadora tiene ${aiPoints}`);
        this.score[1] += this.currentBet;
        this.logEvent(`Computadora ganó ${this.currentBet} punto(s)`);
        ...
        if (winner === 1) {
          this.logEvent(`Computadora tiene ${aiPoints} (ganó por ser mano)`);
          this.logEvent(`Computadora ganó ${this.currentBet} punto(s)`);
        } else {
          this.logEvent(`Computadora tiene ${aiPoints}`);
          this.logEvent(`Jugador ganó ${this.currentBet} punto(s) por ser mano`);
        }
      ...
      for (const button of this.buttons) {
        if (button.isClicked()) {
          switch (button.label) {
            ...
            case "Ir al mazo":
              this.score[1] += 1;
              this.logEvent("Jugador se fue al mazo. Computadora ganó 1 punto");
              this.endHand();
              break;
          }
          return;
